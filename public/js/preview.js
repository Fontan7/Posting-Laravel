...
// aca se podria variar el concatenado en funcion a si se muestra primero la imagen
  ...
  var textH = divGral.concat(divPre).concat(divEncPre).concat(imgUser).concat(divTit).concat('<h5>').concat(usenv).concat('- ').concat(i.name).concat('</h5>').concat(divH).concat('Hace una hora').concat(closeDiv).concat(closeDiv).concat(pPre).concat(closeDiv).concat(img_post).concat(srcFoto).concat('"') .concat('alt="your image">').concat(footPre).concat(closeDiv);
